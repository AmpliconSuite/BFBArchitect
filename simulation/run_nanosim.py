# Adaptation from https://github.com/AmpliconSuite/ecSimulator
...
# this script will run the nanosim pipeline to simulate nanopore reads from amplicon and background genome.
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Call NanoSim to simulate nanopore reads from genome regions.")
    ...
    parser.add_argument("-t", "--num_threads", type=int, help="Number of threads to use in all stages (default=8).",
                        default=8)
    ...
    parser.add_argument("--amp_is_circular", action='store_true', help="Set if the simulated amplicon is circular instead"
                                                                     " of linear.")
    ...
    linear_or_circular = "circular" if args.amp_is_circular else "linear"
    ...
    run_nanosim(args.amplicon_fasta, model_path, args.output_prefix, args.amplicon_coverage, linear_or_circular,
    ...
    if args.background_fasta:
        ...
        background_prefix = args.output_prefix + "_background"
